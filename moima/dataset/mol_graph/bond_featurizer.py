...
class BondFeaturizer:
    ...
    def __call__(self, bond: Bond) -> np.ndarray:
        ...
        for name in self.generator_name_list:
            ...
            feature_fn = get_bond_features_generator(name)
            if name in self.params:
                bond_features += feature_fn(bond, **self.params[name])
            else:
                bond_features += feature_fn(bond)
...
def register_bond_features_generator(
...
def bond_features_union(
    ...
    for generator_name in generator_name_list:
        if generator_name in get_avail_bond_features():
            feature_fn = get_bond_features_generator(generator_name)
            if generator_name in params:
                bond_features += feature_fn(bond, **params[generator_name])
            else:
                bond_features += feature_fn(bond)
        else:
