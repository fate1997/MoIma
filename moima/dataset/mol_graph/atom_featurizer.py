    ...
    def get_atom_repr(self, atom: Atom) -> np.ndarray:
        ...
        for name in self.generator_name_list:
            ...
            if name in self.params:
                feature = get_atom_feature_generator(name)(atom, **self.params[name])
            else:
                feature = get_atom_feature_generator(name)(atom)
            
            if self.return_onehot:
                atom_features += feature
            else:
                if len(feature) == 1:
                    atom_features.append(feature[0])
                else:
                    atom_features.append(feature.index(1))
