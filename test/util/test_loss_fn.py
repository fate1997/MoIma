...
def test_vae_loss():
    ...
    assert loss_dict.keys() == {'recon_loss', 'kl_loss'}
...
def test_loss_fn_factory():
    assert len(LossCalcFactory.avail) > 0
    vae_loss_fn = LossCalcFactory.create(name='vae_loss', ratio=0.7)
    assert isinstance(vae_loss_fn, VAELossCalc)
    vade_loss_fn = LossCalcFactory.create(name='vade_loss', kl_weight=1)
...
def test_regression_loss():
    ...
    for name in loss_names:
        loss_args = {'name':name}
        loss_fn = LossCalcFactory.create(**loss_args)
